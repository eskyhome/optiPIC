...
#    fi
    echo "set xrange[0:0.4]" >> $file
    echo "set yrange[0:0.4]" >> $file
    echo "set terminal png"         >> $file
    echo "set output 'bilde$i.png'"   >> $file
    echo "plot \\" >> $file
    for j in 0 1 2 ; do
        echo " 'p_"$j"_plot"$i"',\\" >> $file 
    done
    echo "'p_3_plot"$i"'" >> $file


#    echo "plot '"$file2"' " >> $file 
    gnuplot $file
...
#mencoder mf://*.png -mf w=800:h=600:fps=5:type=png -ovc lavc -lavcopts vcodec=mpeg4:mbd=2:trell -oac copy -o ../particles.avi
cd ..
